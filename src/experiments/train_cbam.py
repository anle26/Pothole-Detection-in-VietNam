...
"""
🚀 YOLOv8s + CBAM Training Pipeline
This script integrates the Convolutional Block Attention Module (CBAM) into YOLOv8.
...
# 💥 MONKEY-PATCHING: Replace standard C2f with our C2f_CBAM dynamically
apply_cbam_patch()
...
def main():
    ...
    print("=" * 60)
    print("🚀 INITIALIZING YOLOv8s + CBAM TRAINING PIPELINE")
    print("=" * 60)
    ...
    # CBAM   saves to: runs/detect/yolov8s_cbam/
    # ------------------------------------------------------------------
    RUN_PROJECT = os.path.join(str(PROJECT_ROOT), "runs", "detect")
    RUN_NAME = "yolov8s_cbam"
    ...
    # 2. Load Model
    print("\n📦 Loading Pre-trained YOLOv8s and applying CBAM patch...")
    ...
    if os.path.exists(best_weights):
        ...
        src_dst_mappings = [
            ("BoxF1_curve.png", os.path.join(reports_fig_dir, "cbam_F1_curve.png")),
            ("BoxPR_curve.png", os.path.join(reports_fig_dir, "cbam_PR_curve.png")),
            ("confusion_matrix.png", os.path.join(reports_fig_dir, "cbam_confusion_matrix.png")),
            ("results.png", os.path.join(reports_fig_dir, "cbam_results_curve.png")),
            ("results.csv", os.path.join(reports_metric_dir, "cbam_results.csv")),
        ]
        for src_name, dst_path in src_dst_mappings:
            src_path = os.path.join(RUN_DIR, src_name)
            if os.path.exists(src_path):
                shutil.copy(src_path, dst_path)
                print(f"📋 Copied cbam {src_name} -> {dst_path}")
        ...
        # ==============================================================
        print("\n" + "=" * 60)
        print("📈 FINAL EVALUATION RESULTS — YOLOv8s + CBAM")
        print("=" * 60)
