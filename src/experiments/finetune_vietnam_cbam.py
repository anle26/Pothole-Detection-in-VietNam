...
"""
🚀 Fine-Tuning Pipeline: YOLOv8 + CBAM on Vietnam Dataset
This script evaluates and fine-tunes the CBAM model on the Vietnam Dataset.
...
# Import CBAM modules and monkey patch
...
def run_kfold(model_path, kfold_yaml_prefix, out_dir):
    ...
    for k in range(1, 6):
        ...
        run_dir = os.path.join(out_dir, "full_lpft", "yolo_cbam", f"fold_{k}") # IMPORTANT: Keeps EXACT same path
    ...
    print(f"\n[CBAM] K-Fold Avg - mAP50-95: {avg_map:.4f} | P: {avg_p:.4f} | R: {avg_r:.4f}")
...
# ==============================================================================
def main():
    parser = argparse.ArgumentParser(description="Fine-tune YOLOv8+CBAM on Vietnam Dataset")
    ...
    if args.mode == "worker":
        apply_cbam_patch() # Must apply patch before loading model
    ...
    # Pretrained cbam model
    cbam_model_path = os.path.join(base_dir, "runs", "detect", "yolov8s_cbam", "weights", "best.pt")
    if not os.path.exists(cbam_model_path):
        print(f"❌ Cannot find pretrained cbam model at: {cbam_model_path}")
        print("   Please run train_cbam.py first.")
        sys.exit(1)

    print("=" * 60)
    print("🚀 EVALUATING/FINE-TUNING: YOLOv8 + CBAM")
    ...
    run_dir = os.path.join(out_dir, "full_lpft", "yolo_cbam", "fold_1")
    
    metrics = spawn_worker("lpft", cbam_model_path, data_yaml, run_dir, epochs_lp=20, epochs_ft=50)
    
    print(f"\n[CBAM] Fold 1 - mAP50-95: {metrics['map']:.4f} | P: {metrics['precision']:.4f} | R: {metrics['recall']:.4f}")
