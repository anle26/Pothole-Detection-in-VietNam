...
"""
🚀 Fine-Tuning Pipeline: YOLOv8 Baseline on Vietnam Dataset
This script evaluates and fine-tunes the BASELINE model on the Vietnam Dataset.
...
# Explicitly remove SimAM patch to ensure pure baseline
import ultralytics.nn.modules.block as block
import ultralytics.nn.tasks as tasks
from src.models.simam_module import OriginalC2f

def remove_simam_patch():
    # print("🔧 Ensuring Base C2f is used...")
...
def run_kfold(model_path, kfold_yaml_prefix, out_dir):
    ...
    for k in range(1, 6):
        ...
        run_dir = os.path.join(out_dir, "full_lpft", "yolo_base", f"fold_{k}") # IMPORTANT: Keeps EXACT same path
    ...
    print(f"\n[Baseline] K-Fold Avg - mAP50-95: {avg_map:.4f} | P: {avg_p:.4f} | R: {avg_r:.4f}")
...
# ==============================================================================
def main():
    parser = argparse.ArgumentParser(description="Fine-tune YOLOv8 Baseline on Vietnam Dataset")
    ...
    if args.mode == "worker":
        remove_simam_patch() # Guarantee pure baseline
    ...
    if not os.path.exists(base_model_path):
        print(f"❌ Cannot find pretrained baseline model at: {base_model_path}")
        print("   Please run train_baseline.py first.")
        sys.exit(1)

    print("=" * 60)
    print("🚀 EVALUATING/FINE-TUNING: YOLOv8 BASELINE")
    ...
    if args.task in ["all", "zero_shot"]:
        ...
        base_zero_dir = os.path.join(out_dir, "zero_shot", "yolo_base") # IMPORTANT: Path matches perfectly
        spawn_worker("zero_shot", base_model_path, few_shot_yaml, base_zero_dir)
        
    # 2. Few-Shot
    if args.task in ["all", "few_shot"]:
        print("\n[Task 2] FEW-SHOT (50 IMAGES) LP-FT")
        base_few_dir = os.path.join(out_dir, "few_shot", "yolo_base") # IMPORTANT: Path matches perfectly
        spawn_worker("lpft", base_model_path, few_shot_yaml, base_few_dir, epochs_lp=30, epochs_ft=50)
        
    # 3. Full K-Fold
    if args.task in ["all", "kfold"]:
        print("\n[Task 3] 5-FOLD CROSS VALIDATION LP-FT")
        run_kfold(base_model_path, kfold_yaml_prefix, out_dir)
