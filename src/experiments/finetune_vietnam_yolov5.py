...
"""
🚀 Fine-Tuning Pipeline: YOLOv5 on Vietnam Dataset
This script evaluates and fine-tunes the baseline YOLOv5 model on the Vietnam Dataset.
...
def run_kfold(model_path, kfold_yaml_prefix, out_dir):
    ...
    for k in range(1, 6):
        ...
        run_dir = os.path.join(out_dir, "full_lpft", "yolov5s", f"fold_{k}") # IMPORTANT: Keeps EXACT same path
    ...
    print(f"\n[YOLOv5s] K-Fold Avg - mAP50-95: {avg_map:.4f} | P: {avg_p:.4f} | R: {avg_r:.4f}")
...
# ==============================================================================
def main():
    parser = argparse.ArgumentParser(description="Fine-tune YOLOv5s on Vietnam Dataset")
    ...
    # Pretrained model
    yolov5_model_path = os.path.join(base_dir, "runs", "detect", "yolov5s_baseline", "weights", "best.pt")
    if not os.path.exists(yolov5_model_path):
        print(f"❌ Cannot find pretrained YOLOv5 model at: {yolov5_model_path}")
        print("   Please run train_yolov5.py first.")
        sys.exit(1)

    print("=" * 60)
    print("🚀 EVALUATING/FINE-TUNING: YOLOv5s")
    ...
    run_dir = os.path.join(out_dir, "full_lpft", "yolov5s", "fold_1")
    
    metrics = spawn_worker("lpft", yolov5_model_path, data_yaml, run_dir, epochs_lp=20, epochs_ft=50)
    
    print(f"\n[YOLOv5s] Fold 1 - mAP50-95: {metrics['map']:.4f} | P: {metrics['precision']:.4f} | R: {metrics['recall']:.4f}")
