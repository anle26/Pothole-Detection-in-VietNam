...
"""
🚀 YOLOv10s Training Pipeline
This script trains YOLOv10 for pothole detection as a comparison baseline.
...
def main():
    ...
    print("=" * 60)
    print("🚀 INITIALIZING YOLOv10s TRAINING PIPELINE")
    print("=" * 60)
    ...
    RUN_NAME = "yolov10s_baseline"
    RUN_DIR = os.path.join(RUN_PROJECT, RUN_NAME)
    
    print(f"\n📁 Output directory : {RUN_DIR}")
    
    # 2. Load Model
    print("\n📦 Loading Pre-trained YOLOv10s...")
    ...
    if os.path.exists(best_weights):
        ...
        src_dst_mappings = [
            ("BoxF1_curve.png", os.path.join(reports_fig_dir, "yolov10s_F1_curve.png")),
            ("BoxPR_curve.png", os.path.join(reports_fig_dir, "yolov10s_PR_curve.png")),
            ("confusion_matrix.png", os.path.join(reports_fig_dir, "yolov10s_confusion_matrix.png")),
            ("results.png", os.path.join(reports_fig_dir, "yolov10s_results_curve.png")),
            ("results.csv", os.path.join(reports_metric_dir, "yolov10s_results.csv")),
        ]
        for src_name, dst_path in src_dst_mappings:
            src_path = os.path.join(RUN_DIR, src_name)
            if os.path.exists(src_path):
                shutil.copy(src_path, dst_path)
                print(f"📋 Copied yolov10s {src_name} -> {dst_path}")
